...
header("location:".get_site_url()."/dev/bestellen//?pg1=1");
}
...
header("location:".get_site_url()."/dev/bestellen/");
}
...
if ( is_user_logged_in() ) {
	...
	if ( !$_REQUEST['fname'] || !$_REQUEST['sname'] || !$_REQUEST['streetname'] ) {
		echo "<script>
			window.location.href = 'http://www.weclip.nl/dev/bestellen//';
		</script>";exit;
	}
	...
	if ( email_exists($email) ){
		...
		echo "<script>
			window.location.href = 'http://www.weclip.nl/dev/bestellen//?existinglogin=".urlencode($email)."';
		</script>";exit;
		}
...
for($i=0; $i<$nof; $i++) { 
	...
	$server->port = 587;
	$server->smtp = true;
	$server->username = "noreply@weclip.nl";
	$server->password = "H7e8*d8bKw2u";
	$server->host = "smtp.mail.pcextreme.nl";
